...
def get_img_for_predict():
  list_img = list(map(lambda x: read_img(os.path.join(input_data.dir_folder, f'{x}.png')), input_data.RAKURSES))
...
def save_gen_img(img):
  Z =  np.zeros((8, 3088, 2320))
  for i in range(8):
    Z[i, 800:2848, 136:2184] = img[:,:,i]
    cv2.imwrite(os.path.join(input_data.dir_write, f'{str(i)}.jpg'), Z[i])